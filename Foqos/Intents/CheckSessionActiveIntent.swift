...
struct CheckSessionActiveIntent: AppIntent {
  ...
  static var title: LocalizedStringResource = "Check if Lockt Session is Active"
  static var description = IntentDescription(
    "Check if any Lockt blocking session is currently active and return true or false. Useful for automation and shortcuts."
  )
  ...
  func perform() async throws -> some IntentResult & ReturnsValue<Bool> & ProvidesDialog {
    ...
    let dialogMessage =
      isActive
      ? "A Lockt session is currently active."
      : "No Lockt session is active."
